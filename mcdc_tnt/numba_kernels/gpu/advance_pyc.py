                            ...
                            int *number_done)
{
    ...
    if (i < num_part){
        if (p_end_trans[i] == 0){
            if (p_pos_x[i] < 0){
                p_end_trans[i] = 1;
                atomicAdd(&number_done[0], 1);
                }
                
            else if (p_pos_x[i] >= L){
                p_end_trans[i] = 1;
                atomicAdd(&number_done[1], 1);
            }
...
def Advance(p_pos_x, p_pos_y, p_pos_z, p_mesh_cell, dx, p_dir_y, p_dir_z, p_dir_x, p_speed, p_time,
    ...
    number_done = np.zeros(3, dtype=np.int32)
    
    #d_number_done = cuda.to_device(number_done)
    
    AdvanceCuda = mod.get_function("AdvanceCuda")
    
    clever_io = np.array([L, dx, num_part, max_mesh_index], dat_type)
    ...
    while end_flag == 0 and cycle_count < 1000:
        #allocate randoms
        rands = np.random.random(num_part).astype(dat_type)
        
        checker = np.zeros(1, np.int32)
        ...
        AdvanceCuda(d_p_pos_x, d_p_pos_y, d_p_pos_z,
                      ...
                      d_p_end_trans, drv.In(rands), d_mesh_dist_traveled, d_mesh_dist_traveled_squared, drv.InOut(number_done), drv.InOut(checker),
                      block = (threadsperblock, 1, 1), grid=(blockspergrid, 1))
        
        #print(number_done)
        
        #print('Correct raster?    {0}'.format(checker[0] == num_part))
        
        if (sum(number_done) == num_part):
            end_flag = 1
        
        drv.memcpy_dtoh(p_end_trans, d_p_end_trans)
        number_done_2 = np.sum(p_end_trans)
        
        cycle_count += 1
        #print("Number done (atomics): {0}    Number done (classical): {1}".format(sum(number_done), number_done_2))
        
        print("Advance Complete:......{0}%       ({1}/{2})    cycle: {3}".format(int(100*sum(number_done)/num_part), number_done, num_part, cycle_count), end = "\r")
    print()
