...
def FissionsAdd(p_pos_x, p_pos_y, p_pos_z, p_region, p_dir_y, p_dir_z, p_dir_x, p_speed, p_time, p_alive,
                fis_count, nu_new_neutrons, fission_event_index, num_part, particle_speed):
    
    k=0 #index for fission temp vectors
    for i in range(fis_count-1):
        for j in range(nu_new_neutrons):
            # Position
            p_pos_x[k+num_part] = p_pos_x[fission_event_index[i]]
            p_region[k+num_part] = p_region[fission_event_index[i]]
    ...
    return(p_pos_x, p_pos_y, p_pos_z, p_region, p_dir_y, p_dir_z, p_dir_x, p_speed, p_time, p_alive, num_part)